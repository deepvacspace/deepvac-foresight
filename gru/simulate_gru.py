...
DEFAULT_CHECKPOINT = Path(__file__).resolve().parent / \
    "validation_t1" / "gru_t1.pt"
...
class CodesysDiff:
    ...
    def update(self, value: float) -> float:
        ...
        self.filter_out = self.dc * self.filter_out + \
            (1.0 - self.dc) * filter_in
...
class PidCoefSelector:
    ...
    def __init__(
        ...
        self.range_width = (float(max_range) - float(min_range)
                            ) / float(self.points_count)
        if self.range_width == 0.0:
            raise ValueError(
                "PidCoefSelector max_range must differ from min_range.")
...
class ChamberPID:
    def __init__(self, u_min: float = -1.0, u_max: float = 1.0, pid_i_reverse_mul: float = 0.333):
    ...
    def step(
        ...
        if effective_i_coef != 0.0 and abs(delta) < delta_edge:
            self.i_part += (1.0 / float(p_coef)) * \
                (delta * 0.1 / effective_i_coef)

        self.d_part = (1.0 / float(p_coef)) * \
            (float(d_coef) * -float(diff_out))
...
def predict_delta_t1(
    ...
    x_scaled = x_scaler.transform(
        feature_window.reshape(-1, n_features)).reshape(feature_window.shape)

    xb = torch.as_tensor(x_scaled[None, :, :],
