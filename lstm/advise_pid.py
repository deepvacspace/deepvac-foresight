...
LSTM plant model as a simulator, with CEM/random-shooting search over it.
...
Examples:
    ...
    python -m lstm.advise_pid --mode suggest --checkpoint lstm/validation_rollout/promoted.pt
    python -m lstm.advise_pid --mode adapt   --checkpoint lstm/validation_rollout/promoted.pt \
...
from lstm.twin_acceptance import describe_trajectory, warm_start_at  # noqa: E402
...
def build_arg_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="AI advisor: recommend or continuously adapt PID gains via CEM search over the LSTM twin."
    )

    ap.add_argument("--mode", choices=["suggest", "adapt"], default="suggest")
    ap.add_argument("--checkpoint", required=True,
                    help="A rollout-trained checkpoint (lstm/train_lstm_rollout.py's output).")
