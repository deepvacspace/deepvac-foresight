...
(temp, current_pid, candidate_pid), instead of simulating the LSTM twin.
...
1. Generate a labeled dataset: sample (temp, current_pid) states, score
   ...
   lstm.advise_pid.horizon_cost.
2. Train a small MLP (lstm/cost_surrogate.py) to regress cost, then report
...
same horizon lstm/advise_pid.py must be run with.

Example:

    python -m lstm.train_cost_surrogate --checkpoint lstm/validation_rollout/lstm_rollout.pt
"""
...
from lstm.advise_pid import add_cost_args, build_initial_state, horizon_cost
from lstm.cost_surrogate import FEATURE_NAMES, CostSurrogateMLP
from lstm.lstm_common import load_model
from lstm.twin_acceptance import rank_correlation
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--checkpoint", required=True, help="A rollout-trained LSTM checkpoint.")
...
def main() -> None:
    ...
    report = {
        ...
        "lstm_checkpoint": str(Path(args.checkpoint).resolve()),
        "target_normalization": final_args.target_normalization,
