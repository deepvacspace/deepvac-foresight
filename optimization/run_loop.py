...
This script runs the automated TCP test loop using the three-model methodology
from band_gp_bo.py:

1. Fit three band-specific GP models from run history.
2. Suggest a far / mid / near controller.
3. Floor the suggested values to ints before applying them.
4. Execute one full run with band-crossing PID updates.
5. Save samples and run summary.
6. Recompute band metrics, retrain all three models, and repeat.
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.set_defaults(
        ...
        models_out="history/band_gp_models.pkl",
        next_out="history/band_bo_next_params.json",
        suggestions_dir="history/band_bo_suggestions",
    )
    ...
    ap.add_argument("--all-runs-csv", default="history/band_all_runs.csv")
