#!/usr/bin/env python3
"""Live GRU-ranked GP PID controller for the chamber.

The script waits until get_states returns a finite temp_u value, takes that
reading as the experiment start, then repeatedly:
  1. updates the GRU feature window from live chamber states,
  2. predicts candidate futures with the copied GRU model,
  3. chooses the best PID coefficients,
  4. sends them to the chamber PID row.
"""
...
from utils.bo_common import append_rows_csv, make_run_id  # noqa: E402
...
def read_states_with_retries(args: argparse.Namespace) -> Dict[str, float]:
    ...
    for _ in range(max(1, int(args.read_retries) + 1)):
        try:
...
def main() -> None:
    ...
    while True:
        ...
        try:
            ...
            print(f"[run {run_id}] get_states failed ({consecutive_failures}/{args.max_consecutive_failures}): {exc}")
        ...
        if elapsed_s >= next_decision_elapsed:
            ...
            if changed or bool(args.apply_unchanged):
                if not bool(args.dry_run):
                    kp_i, ki_i, kd_i = apply_pid_update(
