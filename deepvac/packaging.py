...
from deepvac.live_window import LiveFeatureWindow
...
def _generate_shared_block(model_type: ModelType) -> str:
    ...
    parts = [
        ...
        "Two starting-window options below, best to worst: LiveFeatureWindow (real\n"
        "telemetry -- push one sample per control tick and wait for `.ready`),\n"
        "initialize_feature_window_from_pid (no telemetry, but physically motivated --\n"
        "see its docstring), and initialize_feature_window (a flat, zero-control\n"
        "window the model never saw in training; kept only for compatibility).\n\n"
        ...
        "",
        "",
        _source_of(_mpc.initialize_feature_window_from_pid),
        "",
        "",
        _source_of(LiveFeatureWindow),
        "",
